...
void CBaseViewModel::CalcIronsights( Vector &pos, QAngle &ang )
{
	...
	float delta( ( gpGlobals->curtime - pWeapon->m_flIronsightedTime ) / 0.15f ); //modify this value to adjust how fast the interpolation is
...
void CBaseViewModel::CalcViewModelLag( Vector& origin, QAngle& angles, QAngle& original_angles )
{
	...
	angles[PITCH] = vOriginalAngles[PITCH];
	return; // dont need whats below
