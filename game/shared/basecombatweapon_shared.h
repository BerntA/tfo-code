...
public:
	...
	// TFO Definitions :
	void DoWeaponFX( void ); // Dynamic effects on weapon!
	virtual int GetOverloadCapacity() { return 3; } // Max shots until gun overheats.
	int m_iShotsFired; // Record how many shots we've fired.
	float m_fIronsightIN;

	// TFO Holster Sequence Logic
	virtual void HandleWeaponSelectionTime(void);
	void StartHolsterSequence(bool bNoSwitch = false); // Starts holster, Holster() just hides and finalizes the prev wep state.
...
public:
	...
	float					GetWeaponFOVOffset( void ) const;
...
public:
	...
	// TFO - Used for removing the remaining clip when reloading.
