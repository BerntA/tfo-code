...
// TFO - Allows us to access m_hl2.local pointer to suitpower!
#if defined( GAME_DLL )
...
//-----------------------------------------------------------------------------
// Purpose: TFO Speed Changes...
//-----------------------------------------------------------------------------
float CGameMovement::ProcessMovementSpeed( CBasePlayer *pPlayer, float flHealth, float flSuitPower )
{
	CBaseCombatWeapon *pWeapon = pPlayer->GetActiveWeapon();

	if ( !pWeapon )
	{
		if ( flHealth > 90 && flHealth < 98 )
			return 80;

		if ( flHealth >= 50 && flHealth < 90 )
			return 70;
	...
	if ( pWeapon->IsIronsighted() && flHealth >= 50 )
	{
		if ( flSuitPower < 50 )
			return 50;
	...
	if ( flHealth >= 50 && flHealth < 90 && !pWeapon->IsIronsighted() )
		return 80;
...
void CGameMovement::ProcessMovement( CBasePlayer *pPlayer, CMoveData *pMove )
{
	...
	float flSuitSpeed = 0.0f;

#if defined( GAME_DLL )
	CHL2_Player *pHL2Player = dynamic_cast< CHL2_Player* >( player );
	flSuitSpeed = pHL2Player->m_HL2Local.m_flSuitPower;
#endif

	mv->m_flMaxSpeed = ProcessMovementSpeed( pPlayer, pPlayer->m_iHealth, flSuitSpeed );
...
void CGameMovement::WalkMove( void )
{
	...
	if ( wishspeed > 200 )
	{
		player->m_bIsRunning = true;
	}
	else
	{
		player->m_bIsRunning = false;
	}
...
{
	// IF we're under 25 suit power we can't jump!
#if defined( GAME_DLL )
	CHL2_Player *pHL2Player = dynamic_cast<CHL2_Player*>(player);

	if ( pHL2Player->m_HL2Local.m_flSuitPower <= 12 ) 
	{
		mv->m_nOldButtons |= IN_JUMP;
		return false;
	}
	...
	// Do a view punch / move! And emit a sound on jump.
	if (gpGlobals->maxClients <= 1)
	{
		player->ViewPunch(QAngle(3, 0, 0));
		player->EmitSound("TFO.Jump");
	}

	// Remove a part of the suit power stamina as we jump.
#if defined( GAME_DLL )
	if ( pHL2Player->m_HL2Local.m_flSuitPower <= 100 )
	{
		pHL2Player->m_HL2Local.m_flSuitPower -= 12;
	}
#endif
...
void CGameMovement::PlayerMove( void )
{
	...
	Duck();
	 ...
	 // Cam Move Punch - Move to the sides. TFO
	 if ( ( mv->m_nButtons & IN_MOVELEFT ) && !movepuncherL )
	{
		movepuncherL = true;
	}

	 if ( ( mv->m_nButtons & IN_MOVERIGHT ) && !movepuncherR )
	{
		movepuncherR = true;
	}
	 ...
	 {
		 movepuncherL = false;
		 player->ViewPunch( QAngle( 0, 0.1f, -0.15f ));
	 }
	 if ( movepuncherR )
	 {
		 movepuncherR = false;
