...
void CBaseViewport::Start( IGameUIFuncs *pGameUIFuncs, IGameEventManager2 * pGameEventManager )
{
	...
	m_GameEventManager->AddListener(this, "display_note", false);
	m_GameEventManager->AddListener(this, "inventory_add", false);
	m_GameEventManager->AddListener(this, "inventory_remove", false);
	m_GameEventManager->AddListener(this, "tfo_map_transition", false);
	m_GameEventManager->AddListener(this, "tfo_map_transition_finished", false);
	m_GameEventManager->AddListener(this, "dialogue_started", false);
	m_GameEventManager->AddListener(this, "tfo_mainmenu_loaded", false);
...
void CBaseViewport::FireGameEvent( IGameEvent * event)
{
	...
	if ( Q_strcmp(type, "display_note") == 0 )
	{
		const char *szFile = event->GetString( "script" );
		GameBaseClient->ShowNote(szFile);
	}

	if ( Q_strcmp(type, "inventory_add") == 0 )
	{
		const char *szFile = event->GetString( "script" );
		GameBaseClient->AddToInventory(szFile);
	}

	if ( Q_strcmp(type, "inventory_remove") == 0 )
	{
		const char *szFile = event->GetString("script");
		GameBaseClient->RemoveItemFromInventory(szFile);
	}

	if ( Q_strcmp(type, "tfo_map_transition") == 0 )
	{
		// Save Temp File:
	...
	if ( Q_strcmp(type, "tfo_map_transition_finished") == 0 )
	{
		...
		GameBaseClient->DeleteAutoSaveFile();
	}

	if ( Q_strcmp(type, "dialogue_started") == 0 )
	{
		bool bOption1 = event->GetInt( "option1" );
		bool bOption2 = event->GetInt( "option2" );
		bool bOption3 = event->GetInt( "option3" );

		const char *szFile = event->GetString( "script" );
		const char *szEnt = event->GetString( "entity" );

		// Open Dialogue + Parse File:
		GameBaseClient->StartDialogueScene(szFile, szEnt, bOption1, bOption2, bOption3);
	}

	if ( Q_strcmp(type, "tfo_mainmenu_loaded") == 0 )
	{
