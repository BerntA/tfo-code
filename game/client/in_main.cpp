...
static  kbutton_t	in_healthkit;
static  kbutton_t	in_knife;
...
void IN_HealthKitDown( const CCommand &args ) { KeyDown( &in_healthkit, args[1] ); }
void IN_HealthKitUp( const CCommand &args ) { KeyUp( &in_healthkit, args[1] ); }
void IN_KnifEDown( const CCommand &args ) { KeyDown( &in_knife, args[1] ); }
void IN_KnifEUp( const CCommand &args ) { KeyUp( &in_knife, args[1] ); }
void IN_DropUp( const CCommand &args ) { KeyUp( &in_drop, args[1] ); }
void IN_DropDown( const CCommand &args ) { KeyDown( &in_drop, args[1] ); }
void IN_HolsterDown( const CCommand &args ) { KeyDown( &in_holster, args[1] ); }
void IN_HolsterUp( const CCommand &args ) { KeyUp( &in_holster, args[1] ); }
...
int CInput::GetButtonBits( int bResetState )
{
	...
	CalcButtonBits( bits, IN_HEALTHKIT, s_ClearInputState, &in_healthkit, bResetState );
	CalcButtonBits( bits, IN_KNIFE, s_ClearInputState, &in_knife, bResetState );
	CalcButtonBits( bits, IN_HOLSTER, s_ClearInputState, &in_holster, bResetState );
...
static ConCommand startobjective( "+objective", IN_ObjecTiveDown );
static ConCommand endobjective( "-objective",  IN_ObjecTiveUp );
static ConCommand starthealthkit( "+healthkit", IN_HealthKitDown );
static ConCommand endhealthkit( "-healthkit",  IN_HealthKitUp );
static ConCommand startknife( "+knife", IN_KnifEDown );
static ConCommand endknife( "-knife",  IN_KnifEUp );
static ConCommand startholster( "+holster", IN_HolsterDown );
static ConCommand endholster( "-holster",  IN_HolsterUp );
