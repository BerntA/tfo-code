...
// -------------------------------------------------------------------------------- //
	...
	IMPLEMENT_CLIENTCLASS_DT(C_BasePlayer, DT_BasePlayer, CBasePlayer)
		...
		RecvPropInt( RECVINFO( m_iInventoryItems ) ),
		RecvPropBool( RECVINFO( m_bHasHealthkit ) ),
		RecvPropBool( RECVINFO( m_bIsTransiting ) ),
		RecvPropBool(RECVINFO(m_bCanDoMeleeAttack)),

		RecvPropBool( RECVINFO( m_bIsRunning ) ),
		RecvPropBool( RECVINFO( m_bCanIronsight ) ),
		RecvPropBool( RECVINFO( m_bShouldLowerWeapon ) ),
		RecvPropFloat( RECVINFO( m_flLoweredWepTime ) ),
		RecvPropBool( RECVINFO( m_bIsInCamView ) ),
...
C_BasePlayer::C_BasePlayer() : m_iv_vecViewOffset( "C_BasePlayer::m_iv_vecViewOffset" )
{
	...
	// TFO
	m_bIsWeaponSelectionActive = false;
	m_bIsRunning = false;
...
// TFO Warn
static ConVar cl_player_render_mirror("cl_player_render_mirror", "1", FCVAR_ARCHIVE, "Enable or disable player mirror/monitor rendering", true, 0, true, 1);
 
bool g_bMirrorRender = false;

int C_BasePlayer::DrawModel( int flags )
{
	// If we don't want mirror render then return! Exception is thirdperson. We'll always want our model rendered in thirdperson @ dialogue system thirdpers.
	if(!cl_player_render_mirror.GetBool() && !input->CAM_IsThirdPerson())
	{
		return 0;
	}

	// If the mirror isn't rendering and we are not in thirdperson and we're the local player then don't render the model!
	if(!g_bMirrorRender && !input->CAM_IsThirdPerson() && IsLocalPlayer() )
	{
		return 0;
	}
...
void CC_ToggleIronSights( void )
{
	...
	if (pPlayer->m_bIsRunning || !pPlayer->m_bCanIronsight || pPlayer->m_bShouldLowerWeapon || pPlayer->m_bIsInCamView || !pPlayer->GetGroundEntity() || !pPlayer->IsAlive() || !pPlayer->m_bCanDoMeleeAttack)
		return;
	...
	engine->ServerCmd( "ironsight_toggle" );
}
 
static ConCommand ironsight_toggle("ironsight_toggle", CC_ToggleIronSights);
