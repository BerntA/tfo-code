...
{
	{ "item_ammo_pistol",			"Pistol",		8,		0.5f },
	{ "item_ammo_smg1",				"SMG1",			30,			0.4f },
	{ "item_ammo_smg1_grenade",		"SMG1_Grenade", 1, 0.0f },
	{ "item_ammo_ar2",				"AR2",			0,			0.0f },
	{ "item_box_buckshot",			"Buckshot",		30,		0.0f },
	{ "item_rpg_round",				"RPG_Round",	SIZE_AMMO_RPG_ROUND,	0.0f },
	{ "weapon_stiel",				"Grenade",		1,						0.1f },
	{ "item_ammo_357",				"357",			6,			0.0f },
	{ "item_ammo_crossbow",			"XBowBolt",		1,		0.0f },
	{ "item_ammo_ar2_altfire",		"AR2AltFire",	0,	0.0f },
};
...
void CItem_DynamicResupply::ComputeHealthRatios( CItem_DynamicResupply* pMaster, CBasePlayer *pPlayer, int iDebug, SpawnInfo_t *pSpawnInfo )
{
	for ( int i = 0; i < NUM_HEALTH_ITEMS; i++ )
	{
		...
		if ( i == DS_HEALTH_INDEX )
		{
			...
			float flCurrentHealth = pPlayer->GetHealth() + (pSpawnInfo[i].m_iPotentialItems * 50.0f);
		...
		else if ( i == DS_ARMOR_INDEX )
		{
			...
			else
			{
				...
				float flCurrentArmor = pPlayer->ArmorValue() + (pSpawnInfo[i].m_iPotentialItems * 25.0f);
