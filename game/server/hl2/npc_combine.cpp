...
void CNPC_Combine::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if( GetActivity() == ACT_RANGE_ATTACK2 )
	{
		if( m_iLastAnimEventHandled != COMBINE_AE_GREN_TOSS )
		{
			...
			CBaseEntity *pItem = DropItem( "stiel_ammo", vecStart, RandomAngle(0,360) );
...
{
	if (FClassnameIs(pWeapon, "weapon_mp40") || FClassnameIs(pWeapon, "weapon_thompson") || FClassnameIs(pWeapon, "weapon_g43")
		|| FClassnameIs(pWeapon, "weapon_stg44") || FClassnameIs(pWeapon, "weapon_schienzel"))
	{
		return WEAPON_PROFICIENCY_GOOD;
	}
	else if( FClassnameIs( pWeapon, "weapon_p38" ) )
	{
		return WEAPON_PROFICIENCY_VERY_GOOD;
	}
	else if (FClassnameIs(pWeapon, "weapon_svt40") || FClassnameIs(pWeapon, "weapon_k98ns") || FClassnameIs(pWeapon, "weapon_panzer"))
	{
