...
//
//=============================================================================//
// Bernt: This has been modified to no longer have any headcrab stuff.
...
ConVar zombie_ambushdist( "zombie_ambushdist", "20000" );
...
void CNPC_BaseZombie::PrescheduleThink( void )
{
	...
	if ( CanOpenDoors() )
	{
		// Open doors if we're close enuff:
		CBaseEntity *pDoor = NULL;
		if ( !pDoor )
		{
			pDoor = gEntList.FindEntityByClassnameNearest( "func_door_rotating", GetAbsOrigin(), 120.0f );
			if ( !pDoor )
				pDoor = gEntList.FindEntityByClassnameNearest( "prop_door_rotating", GetAbsOrigin(), 120.0f );
		}

		// If we find a door and we're movin around + the player is in a reasonable range we'll proceed to open the door to figure out if he's hiding behind it.
		if ( pDoor && ( IsMoving() || ( PlayerInRange( GetAbsOrigin(), 200.0f ) != NULL ) ) )
		{
			CBaseDoor *pRealDoor = dynamic_cast< CBaseDoor* > ( pDoor );
			if ( pRealDoor )
			{
				if ( !pRealDoor->m_bLocked )
					pRealDoor->DoorGoUp();
			}
			...
			if ( pPropDoor )
			{
				if ( !pPropDoor->IsDoorLocked() && pPropDoor->IsDoorClosed() && !pPropDoor->m_bIsOpening )
