...
public:
	...
	// Weapon, Animation, Running, Ironsight, MISC Effects...
	//*
	// Check Activity:
	void CheckAnimationEvents( CBaseCombatWeapon *pCurrentWeapon );
	// Check New Buttons:
	void CheckButtonsDown( void );
	// Play an animation on the current weapon.
	void PlayAnimation( CBaseCombatWeapon *pWeapon, int iActivity, float flOverrideDuration = 0.0f );
	// TFO Anim Event Handlers:
	// Are we running?
	CNetworkVar( bool, m_bIsRunning );
	// Should be swim?
	bool m_bShouldSwim;
	// Can we ironsight?
	CNetworkVar( bool, m_bCanIronsight );
	// Should I lower my weapon?
	CNetworkVar( bool, m_bShouldLowerWeapon );
	// Current time until lowering weapon anims are done playing.
	CNetworkVar( float, m_flLoweredWepTime );
	// Are we in any other cam view mode?
	CNetworkVar( bool, m_bIsInCamView );
	Vector vecOldPlrPos; // Used for storing player origin before enabling cams. This also allows seeing rain / snow while in cam. Todo: Increase render distance for rain when in cam?
	QAngle angOldPlrAng;
	// Donation & End Game:
	bool m_bCanPickupRewards;
	// Bash Fail-Safe
	bool m_bCanBash;
...
public:
	...
	{
		HEALTH_STATE_NEUTRAL = 0,
		HEALTH_STATE_REGEN,
		HEALTH_STATE_BLEEDING,
		HEALTH_STATE_DYING,
	};

	float m_flHealthUpdateTime;
	int m_iHealthState;
...
public:
	...
	int GetInventoryItemCount() { return m_iInventoryItems; }

	// TFO Healthkit:
	CNetworkVar( bool, m_bHasHealthkit );

	// TFO Transiting:
	CNetworkVar( bool, m_bIsTransiting );

	// Melee/Bash handling
	CNetworkVar(bool, m_bCanDoMeleeAttack);
