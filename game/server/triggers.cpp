...
void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	if ( g_debug_transitions.GetInt() == 0 )
	{
		...
		if ( pClient )
			pClient->ProcessTransition();

		SetThink( &CChangeLevel::DoChangeLevel );
...
void CTriggerCamera::Enable( void )
{
	...
	// if the player was already under control of a similar trigger, disable the previous trigger.
	{
		...
		if (pPrevViewControl && pPrevViewControl != pPlayer)
		{
			...
			if ( pOtherCamera )
			{
				...
				{
					// what the hell do you think you are doing? : I wouldn't expect that Valve would code something that bites itself in the back. ~Bernt
	...
	pPlayer->m_bIsInCamView = true;
	pPlayer->vecOldPlrPos = pPlayer->GetAbsOrigin();
	pPlayer->angOldPlrAng = pPlayer->GetAbsAngles();

	pPlayer->SetAbsOrigin( this->GetAbsOrigin() );
	pPlayer->SetAbsAngles( this->GetAbsAngles() );
...
void CTriggerCamera::Disable( void )
{
	if ( m_hPlayer && m_hPlayer->IsAlive() )
	{
		...
		((CBasePlayer*)m_hPlayer.Get())->m_bIsInCamView = false;
		((CBasePlayer*)m_hPlayer.Get())->SetAbsOrigin( ((CBasePlayer*)m_hPlayer.Get())->vecOldPlrPos );
		((CBasePlayer*)m_hPlayer.Get())->SetAbsAngles( ((CBasePlayer*)m_hPlayer.Get())->angOldPlrAng );
		((CBasePlayer*)m_hPlayer.Get())->RemoveEffects( EF_NODRAW );
...
void CTriggerCamera::MoveThink(void)
{
	if (m_hTarget)
	{
		Disable();
